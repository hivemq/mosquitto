...
static void free_discovery_entry(discovery_entry_t *entry)
{
	if (entry) {
		free(entry->topic);
		free(entry->payload_sample);
		free(entry);
	}
}

void pulse_discovery_init(int max_entries)
{
	g_max_entries = max_entries;
	mosquitto_log_printf(MOSQ_LOG_INFO, "Pulse: Discovery module initialized (max_entries=%d)", max_entries);
}
...
bool pulse_discovery_process_message(const char *topic, const void *payload, size_t payload_len)
{
	...
	if (entry) {
		...
		if (payload && payload_len > 0) {
			...
			entry->payload_sample = malloc(sample_len + 1);
		...
		if (g_max_entries > 0 && (int)g_discovery_count >= g_max_entries) {
		...
		/* Create new entry */
		entry = calloc(1, sizeof(discovery_entry_t));
		if (!entry) {
			pthread_mutex_unlock(&g_discovery_mutex);
			return false;
		}
		...
		mosquitto_log_printf(MOSQ_LOG_INFO, "Pulse: Discovered ungoverned topic: %s", topic);
	}
...
void pulse_discovery_clear(void)
{
	...
	mosquitto_log_printf(MOSQ_LOG_INFO, "Pulse: Cleared all discovered topics");
}

void pulse_discovery_cleanup(void)
{
	pulse_discovery_clear();
	mosquitto_log_printf(MOSQ_LOG_INFO, "Pulse: Discovery module cleanup complete");
}
